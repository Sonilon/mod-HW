...
public class ChatGuardConfig {
    ...
    public String violationFormat = "§c[ChatGuard] §fНарушитель: §e{nick} §7— §c{word}";
    public String muteCommand = "/tempmute {nick} {time} {reason}";

    // Кастомный regex для парсинга ника (группа 1 = ник)
    public String nickRegex = "";
    ...
    public static class Trigger {
        ...
        public Trigger(String word, String time, String reason) {
            this.word   = word;
            this.time   = time;
            this.reason = reason;
        }
    ...
    public static void load() {
        if (Files.exists(CONFIG_PATH)) {
            ...
            INSTANCE = new ChatGuardConfig();
            INSTANCE.triggers.add(new Trigger("реклама", "1h", "Реклама сторонних ресурсов"));
            INSTANCE.triggers.add(new Trigger("мат", "30m", "Нецензурная брань"));
            save();
        }
    }
